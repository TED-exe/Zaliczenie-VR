...
{
    [Header("Car Setting")] [SerializeField]
    private Transform[] allWheelsTransform;
    ...
    [Header("Controll Setting")] [SerializeField]
    ...
    [Header("Flip Car Setting")] [SerializeField]
    private Transform flipCheckRaycast;
    ...
    private void FixedUpdate()
    {
        ...
        for (int i = 0; i < allWheelsTransform.Length; i++)
        {
            ...
            {
                Debug.DrawLine(allWheelsTransform[i].position,
                    allWheelsTransform[i].position + Vector3.down * springLeanght, Color.white);
            }
        }
    }
    ...
    {
        if (Physics.Raycast(selectedTireTransform.position, Vector3.down, out var hitPoint, springLeanght,
                whatIsGround))
    ...
    private void SuspesionCalculate(Transform selectedTireTransform, RaycastHit hitPlace)
    {
        ...
        Debug.DrawLine(selectedTireTransform.position,
            selectedTireTransform.position + Vector3.down * hitPlace.distance, Color.red);
        Debug.DrawLine(selectedTireTransform.position, selectedTireTransform.position + Vector3.up * force,
            Color.green);
    }
    ...
    public void FlipCar()
    {
        if (Physics.Raycast(flipCheckRaycast.position, flipCheckRaycast.up, 0.1f, whatIsGround))
        {
            ...
            transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.identity,
                flipRotationSpeed * Time.fixedDeltaTime);
        }
    }
    ...
    private void Steering(Transform selectedWheeleTransform)
    {
        ...
        carRigidbody.AddForceAtPosition(steeringDir * wheelesMass * desiredAcceleration,
            selectedWheeleTransform.position);
    }
